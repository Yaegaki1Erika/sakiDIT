...
class Draft_Attention(nn.Module):
    ...
    def forward(
        ...
        mask_1 = self.attention_percentile_mask_headwise(attn_1, 1 - sparsity_ratio)
        mask_2 = self.attention_percentile_mask_headwise(attn_2, 1 - sparsity_ratio)

        half_bsz = (cu_seqlens_q.numel() - 1) // 2
        mask_1 = mask_1.unsqueeze(0).repeat(half_bsz, 1, 1, 1).bool()
        mask_2 = mask_2.unsqueeze(0).repeat(half_bsz, 1, 1, 1).bool()

        qv1, kv1 = mask_1.shape[2], mask_1.shape[3]
        qv2, kv2 = mask_2.shape[2], mask_2.shape[3]

        # ✅ 创建完整 blockmask，默认全部为 1（允许）
        base_blockmask = torch.ones(
            (half_bsz * 2, num_heads, q_block_num, k_block_num),
            dtype=torch.bool,
            device=q.device,
        )

        # ✅ 只填入合法区域，不越界
        if qv1 <= q_block_num and kv1 <= k_block_num:
            base_blockmask[:half_bsz, :, :qv1, :kv1] = mask_1
        else:
            raise ValueError(f"mask_1 shape ({qv1}, {kv1}) > allowed block size ({q_block_num}, {k_block_num})")

        if qv2 <= q_block_num and kv2 <= k_block_num:
            base_blockmask[half_bsz:, :, :qv2, :kv2] = mask_2
        else:
            raise ValueError(f"mask_2 shape ({qv2}, {kv2}) > allowed block size ({q_block_num}, {k_block_num})")
